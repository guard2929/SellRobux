...
def login_view(request):
    if request.method == 'POST':
        ...
        if user is not None and user.is_active:
            ...
            return render(request, 'login.html', {'error': 'Неверные данные или аккаунт не активирован'})
...
def confirm_code(request):
    if request.method == 'POST':
        ...
        if not registration_data:
            return render(request, 'confirm_code.html', {'error': 'Сессия истекла, повторите регистрацию'})
...
def wallet_withdraw(request):
    if request.method == 'POST':
        ...
        if not all([account_id, amount, wallet_address, withdraw_method]):
